...
async function seedData() {
  ...
  if (userCount === 0) {
    ...
    await p.query(
      ...
      ['admin@railflow.com', hashedPwd, 'Admin']
    );
    await p.query(
      'INSERT INTO users (email, password_hash, role) VALUES ($1, $2, $3)',
      ['superadmin@railflow.com', hashedPwd, 'Super Admin']
    );
    const passengerRes = await p.query(
      'INSERT INTO users (email, password_hash, role) VALUES ($1, $2, $3) RETURNING id',
      ['passenger@railflow.com', hashedPwd, 'Passenger']
    );

    const passengerId = passengerRes.rows[0]?.id;
  }

  // Seed wallet for passenger@railflow.com if not already seeded
  const walletCountRes = await p.query('SELECT COUNT(*) as count FROM wallets');
  const walletCount = Number(walletCountRes.rows[0].count);
  if (walletCount === 0) {
  ...
  // Seed a confirmed booking for E2E tests if bookings empty
  const bookingCountRes = await p.query('SELECT COUNT(*) as count FROM bookings');
  const bookingCount = Number(bookingCountRes.rows[0].count);
  if (bookingCount === 0) {
    const passengerExists = await p.query("SELECT id FROM users WHERE email = 'passenger@railflow.com'");
    const uid = passengerExists.rows[0]?.id;
    if (uid) {
      ...
      const bookingRes = await p.query(
        ...
        [uid, '12951', bookingPnr, 2200.0, JSON.stringify([
          { name: 'John Doe', age: 30, gender: 'M', aadhaar: '123412341234' },
          { name: 'Jane Doe', age: 28, gender: 'F', aadhaar: '567856785678' }
        ])]
      );
      const bookingId = bookingRes.rows[0]?.id;
      if (bookingId) {
        await p.query(
          `INSERT INTO payments (user_id, booking_id, transaction_id, amount, payment_method, status)
           VALUES ($1, $2, $3, $4, $5, 'SUCCESS')`,
          [uid, bookingId, 'TXN' + Date.now() + 'SEED', 2200.0, 'UPI']
        );
        await p.query(
          `UPDATE seats SET status = 'BOOKED', booking_id = $1
           WHERE train_number = $2 AND coach_label = $3 AND seat_number IN (1, 2)`,
          [bookingId, '12951', 'A1']
        );
      }
    }
  ...
  const trainCount = Number(trainCountRes.rows[0].count);
  ...
  if (trainCount === 0) {
    ...
    const trainsToSeed = [
      ...
      },
      {
        ...
        to_station: 'Trivandrum Central (TVC)',
      ...
      },
      {
        ...
        from_station: 'Bangalore City (SBC)',
    ...
    for (const t of trainsToSeed) {
      ...
      // Seed Seats for this train
      // Coaches: A1 (1A), B1 (3A), S1 (SL)
      const coachConfigs = [
  ...
  const stationCount = Number(stationCountRes.rows[0].count);
  ...
  // Seed train routes (stops) if empty
  const routeCountRes = await p.query('SELECT COUNT(*) as count FROM train_routes');
  const routeCount = Number(routeCountRes.rows[0].count);

  if (routeCount === 0) {
    console.log('Seeding train routes into PostgreSQL...');

    // 12951 Mumbai Rajdhani: MMCT -> BCT -> ST -> BRC -> BPL -> VGLJ -> NDLS
    const route12951 = [
    ...
    for (const r of route12951) {
      await p.query(
        `INSERT INTO train_routes (train_number, station_code, stop_number, arrival_time, departure_time, distance_km, day_count)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        ['12951', r.station, r.stop, r.arrival, r.departure, r.dist, r.day]
      );
    }

    // 12626 Kerala Express: NDLS -> LKO -> PNBE -> NGP -> KRNT -> MAS -> TVC
    const route12626 = [
    ...
    for (const r of route12626) {
      await p.query(
        `INSERT INTO train_routes (train_number, station_code, stop_number, arrival_time, departure_time, distance_km, day_count)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        ['12626', r.station, r.stop, r.arrival, r.departure, r.dist, r.day]
      );
    }

    // 22691 Rajdhani: SBC -> NGP -> BPL -> NZM (Hazrat Nizamuddin)
    const route22691 = [
    ...
    for (const r of route22691) {
      await p.query(
        `INSERT INTO train_routes (train_number, station_code, stop_number, arrival_time, departure_time, distance_km, day_count)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        ['22691', r.station, r.stop, r.arrival, r.departure, r.dist, r.day]
      );
    }
  }
  ...
  const eventCount = Number(eventCountRes.rows[0].count);

  if (eventCount === 0) {
    console.log('Seeding events into PostgreSQL...');
    const events = [
      { name: 'A.R. Rahman Live in Concert', category: 'CONCERT', venue: 'Jawaharlal Nehru Stadium', city: 'Delhi', description: 'Experience the magic of A.R. Rahman live', date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), door_open: null, base_price: 1500, total_seats: 200, available_seats: 200 },
      { name: 'IPL Final 2026', category: 'SPORT', venue: 'Wankhede Stadium', city: 'Mumbai', description: 'IPL 2026 Grand Finale', date: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString(), door_open: null, base_price: 5000, total_seats: 500, available_seats: 500 },
      { name: 'Shakespeare in the Park', category: 'THEATRE', venue: 'Central Park', city: 'Bengaluru', description: 'A Midsummer Night\'s Dream', date: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString(), door_open: null, base_price: 800, total_seats: 100, available_seats: 100 },
    ];
    for (const e of events) {
      const eventRes = await p.query(
        `INSERT INTO events (name, category, venue, city, description, date, base_price, total_seats, available_seats, status)
        ...
        for (let si = 0; si < sections.length; si++) {
          for (const row of rows) {
            ...
            for (let sn = 1; sn <= seatsPerRow; sn++) {
              await p.query(
                `INSERT INTO event_seats (event_id, section, row_label, seat_number, price, status)
  ...
  const couponCount = Number(couponCountRes.rows[0].count);
  if (couponCount === 0) {
    console.log('Seeding coupons...');
    await p.query(
      `INSERT INTO coupons (code, discount_percent, discount_max_amount, min_cart_value, usage_limit, expires_at, status)
  ...
  // Seed train coaches (composition) if empty
  const coachCountRes = await p.query('SELECT COUNT(*) as count FROM train_coaches');
  const coachCountN = Number(coachCountRes.rows[0].count);
  if (coachCountN === 0) {
    console.log('Seeding train coach composition...');
    const coachData: { train: string; coaches: { class: string; label: string; pos: number; seats: number }[] }[] = [
      { train: '12951', coaches: [
        { class: '1A', label: 'A1', pos: 1, seats: 9 },
        { class: '3A', label: 'B1', pos: 2, seats: 18 },
        { class: 'SL', label: 'S1', pos: 3, seats: 18 },
      ]},
      { train: '12626', coaches: [
        { class: '1A', label: 'A1', pos: 1, seats: 9 },
        { class: '3A', label: 'B1', pos: 2, seats: 18 },
  ...
  // Update train_routes with platform numbers if not set
  await p.query(
    "UPDATE train_routes SET platform = '3' WHERE train_number = '12951' AND station_code = 'MMCT' AND platform IS NULL"
  );
  await p.query(
    "UPDATE train_routes SET platform = '2' WHERE train_number = '12951' AND station_code = 'NDLS' AND platform IS NULL"
  );
  await p.query(
    "UPDATE train_routes SET platform = '1' WHERE train_number = '12626' AND station_code = 'NDLS' AND platform IS NULL"
  );
  await p.query(
    "UPDATE train_routes SET platform = '4' WHERE train_number = '12626' AND station_code = 'TVC' AND platform IS NULL"
  );
  await p.query(
    "UPDATE train_routes SET platform = '5' WHERE train_number = '22691' AND station_code = 'SBC' AND platform IS NULL"
  );
  await p.query(
    "UPDATE train_routes SET platform = '3' WHERE train_number = '22691' AND station_code = 'NZM' AND platform IS NULL"
  );

  // Seed live train status if empty
  const liveCountRes = await p.query('SELECT COUNT(*) as count FROM live_train_status');
  const liveCount = Number(liveCountRes.rows[0].count);

  if (liveCount === 0) {
    console.log('Seeding live train status...');
    await p.query(
      `INSERT INTO live_train_status (train_number, current_station, status, delay_minutes, speed_kmh, next_station, expected_arrival)
       VALUES ($1, $2, $3, $4, $5, $6, NOW() + INTERVAL '1 hour')`,
      ['12951', 'ST', 'DEPARTED', 0, 95, 'BRC']
    );
    await p.query(
      `INSERT INTO live_train_status (train_number, current_station, status, delay_minutes, speed_kmh, next_station, expected_arrival)
       VALUES ($1, $2, $3, $4, $5, $6, NOW() + INTERVAL '2 hours')`,
      ['12626', 'PNBE', 'DEPARTED', 15, 80, 'NGP']
    );
    await p.query(
      `INSERT INTO live_train_status (train_number, current_station, status, delay_minutes, speed_kmh, next_station, expected_arrival)
       VALUES ($1, $2, $3, $4, $5, $6, NOW() + INTERVAL '3 hours')`,
      ['22691', 'SBC', 'ON_TIME', 0, 110, 'KRNT']
    );
  }
}
